...
| TagInput은 controlled 컴포넌트                      | `[정상] TagInput — should render tag chips for each item in tags prop`                                                                                             | ✅        |
...
| 저장 시 `updateNote({ ...note, tags })` 호출        | `[정상] handleSave — should call updateNote with current tags when saving edited note`                                                                             | ✅        |
| 저장 후 NoteItem에 변경된 태그 반영                 | `[정상] NoteItem — should display updated tags after save`                                                                                                         | ✅        |
